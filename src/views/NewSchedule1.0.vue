      ...
      <div class="row justify-center flex">
        <div class="col-11 q-py-lg">
          ...
          >
            <template v-slot:eventContent="arg">
              <div class="row justify-center flex">
                <template
                  v-if="
                    !tooltipsEnabled || !!arg.event.extendedProps?.autogenerated
                  "
                >
                  <span
          ...
          <div
            ...
            v-if="tooltipsEnabled && diasProgramados.length > 0"
          >
            <q-btn
...
function gotoDateSafe(isoYmd) {
  if (!isoYmd) return;
  nextTick(() => {
    const api = calendarRef.value?.getApi?.();
    if (api) api.gotoDate(isoYmd); // isoYmd tipo '2025-11-03'
  });
}
...
function normalizeValidatedEvents(list) {
  return (list || []).map((ev, i) => {
    ...
    // IMPORTANTE: si viene un 'autogenerated' arriba, bórralo
    // para que FullCalendar no lo reinyecte/pise tu valor.
    if ('autogenerated' in clean) delete clean.autogenerated;

    const id = clean.id || `val-${i}-${ymdFromAny(clean.start) || ''}`;

    // marca SIEMPRE como NO-autogenerado en el resultado validado
    clean.extendedProps = {
      ...(clean.extendedProps || {}),
      autogenerated: false,
    };
...
onBeforeMount(async () => {
  ...
  if (idSchedule.value) {
    ...
    setTimeout(() => {
      ...
      gotoDateSafe(data.fstart || ymdFromAny(data.events?.[0]?.start));
...
async function validateInfo() {
  ...
  const data = {
    ...
    tstart: tStart,
    tend: tEnd,
    days: selectedDaysForApi.value, // días de la semana
...
function toIsoYmd(v) {
  ...
  if (/^\d{4}\/\d{2}\/\d{2}$/.test(s)) return s.replaceAll('/', '-'); // Quasar a veces usa /
  if (/^\d{4}-\d{2}-\d{2}$/.test(s)) return s;
  return ymdFromAny(s); // por si viene tipo 2025-11-03T00:00...
}

function computeRange() {
  // 1) si venimos de consult3.0 (fechas explícitas)
  if (explicitDates.value?.length) {
    const sorted = explicitDates.value.map(toIsoYmd).filter(Boolean).sort();
    if (sorted.length)
      return { fstart: sorted[0], fend: sorted[sorted.length - 1] };
  }

  // 2) inputs normales
  const a = toIsoYmd(fStart.value);
  const b = toIsoYmd(fEnd.value);
  if (a && b) return { fstart: a, fend: b };

  // 3) fallback: lo saco de los eventos (por si todo lo demás está nulo)
  const fromEvents = (diasProgramados.value || [])
    .map((e) => toIsoYmd(e.start))
    .filter(Boolean)
    .sort();

  if (fromEvents.length)
    return { fstart: fromEvents[0], fend: fromEvents[fromEvents.length - 1] };

  return { fstart: null, fend: null };
}

async function saveData() {
  loading.value = true;
  try {
    const { fstart, fend } = computeRange();

    if (!fstart || !fend) {
      $q.notify({
        type: 'negative',
        message: 'Faltan fstart/fend. No voy a mandar un schedule inválido.',
      });
      return;
    }
    ...
    const dataShedule = {
      ...
      tend: timeEnd.value,
      events: diasProgramados.value,
    };

    const res = await useSchedules.postSchedules(dataShedule);

    if (res?.status <= 201) clearFields();
  } catch (err) {
    ...
    $q.notify({
      ...
      message:
        ...
        'Error guardando. Revisa backend/Network.',
    });
  } finally {
    loading.value = false;
  }
...
async function updateData() {
  ...
  try {
    ...
    const res = await useSchedules.putSchedules(idSchedule.value, dataSchedule);
    ...
    $q.notify({
      ...
      message:
        err?.response?.data?.message ||
        'Error actualizando. Revisa backend/Network.',
    });
  } finally {
    loading.value = false;
  }
